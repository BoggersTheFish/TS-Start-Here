...
| [TS-Reasoner-v0](https://github.com/BoggersTheFish/TS-Reasoner-v0) | TS-Core-backed typed tension traces, candidate containment, exported TensionLM-style ingestion, deeper-chain repair, and a tiny learned candidate model under typed verifier authority. | Best current runnable receipt path. |
...
```text
TS-Start-Here -> TS-Reasoner-v0 v2.0.0 -> learned candidate model receipt -> limitations -> typed verifier boundary
```
...
4. Run TS-Reasoner-v0 and inspect its v2.0.0 learned candidate model receipt.
5. Review the v1.0.0 through v2.0.0 ladder before treating learned/model outputs as anything more than candidate data.
